/* ------------------------
 * BEGIN SOURCE FILE: biogps/core_dispatch.js 
 */

// core_dispatch.js
// Authors: Chunlei Wu (cwu@gnf.org)
//          Marc Leglise (mleglise@gnf.org)
// Created: Feb 26, 2009
...
coreDispatcher = {
    ...
    // Special handling is used for two possible "topquery" field names.
    // While this seems completely counter-intuitive, it's because we have the
    // top search box twice in index_ext.html, once each for welcome and main UI.
    // If the input field in the form is given the same "name" attribute for both
    // instances, IE6/7 then chokes when trying to dispatch actions.  This is an
    // incredibly weird bug that we're just working around here.
    doTopBarSearch: function(form, evt){
        if (evt) {evt.cancelBubble=true;}

        var _query;
        if (form.topquery) {
            _query = form.topquery.value.trim();
            form.topquery.value = '';
            form.topquery.blur();
        }
        else {
            _query = form.topquery2.value.trim();
            form.topquery2.value = '';
            form.topquery2.blur();
        }
...
/* END OF FILE: biogps/core_dispatch.js */
