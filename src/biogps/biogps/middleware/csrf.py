"""
This is a modified version of django.middleware.csrf module.
Basically, it overwrites the referer-check to hostname-check only.
See comments in the code for details.

Chunlei Wu 08/12/2010
"""
"""
...
#                                  (referer, good_referer))
                ...
                #########################################################################
                # As above comments said, the original check is too strict indeed.
                # so we changed it to check hostname only
                good_hostname = request.get_host()
                referer_hostname = urlparse.urlparse(referer).hostname
                if not (referer_hostname == good_hostname):
                    return reject("Hostname checking failed - %s does not match %s." %
                                  (referer_hostname, good_hostname))
                #########################################################################
