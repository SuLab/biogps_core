from biogps.dataset.models import BiogpsDataset
...
# Time frames
now = timezone.datetime.utcnow()  # using utc time as the LOG SERVER logs UTC time.
time_frames = ['weekly', 'monthly', 'all_time']
one_mo = now - timezone.timedelta(days=30)
prev_wk = now - timezone.timedelta(weeks=1)
