...
def _extend_user(user):
    ...
    def get_absolute_url(user):
        ...
        return ('apps.bgprofile.view', [str(user.id), clean_username(user.username)])
    ...
    setattr(user, 'clean_username', new.instancemethod(get_clean_username, user, User))
    ...
    setattr(user, 'display_name', new.instancemethod(get_display_name, user, User))
    ...
    setattr(user, 'can_share', new.instancemethod(can_share, user))
...
#                self.groups.filter(name='adam').count()==0)
    ...
    setattr(user, 'add_group_by_name', new.instancemethod(add_group_by_name, user))
    ...
    setattr(user, 'grant_openid', new.instancemethod(grant_openid, user))
    ...
    def account_type(self):
        ...
        return "BioGPS User"
    setattr(user, 'account_type', new.instancemethod(account_type, user))
    ...
    setattr(user, 'is_friends_with', new.instancemethod(is_friends_with, user))
    ...
    def invite_status_with(self, user2):
        ...
        if invite:
            ...
            return False
    setattr(user, 'invite_status_with', new.instancemethod(invite_status_with, user))
    ...
    def is_migrated(self):
        try:
            ...
            return False
    setattr(user, 'is_migrated', new.instancemethod(is_migrated, user))
