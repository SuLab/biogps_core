...
#        db_table = 'www_biogpsgenereportlayout'
    ...
    def get_layout_data(self, loadplugin=False):
        ...
        if loadplugin or self.loadplugin:
            return [dict(id=p.plugin.id,
                         ...
                         url=p.plugin.url,
                         author=smart_unicode(p.plugin.owner.get_valid_name(), strings_only=False),
    ...
    def save_layout_data(self, layout_data):
        ...
        """
        if type(layout_data) in [types.ListType, types.TupleType]:
            self.plugins.clear()
...
try:
    tagging.register(BiogpsGenereportLayout)
