...
class Command(NoArgsCommand):
    ...
    def handle_noargs(self, **options):
        ...
        # Datasets parent directory
            ...
            datasets = [{
                            'ds': 'U133AGNF1B.gcrma.csv',
                            'default': True, 'id': 1, 'dir': b, 'name': 'GeneAtlas U133A, gcrma',
                            'delimiter': ',', 'color': 'U133AGNF1B.gcrma.coloring.csv',
                        ...
                        {
                            'ds': 'NCI60_U133A_20070815.raw.csv',
                        ...
                        {
                            'ds': 'GNF1M_plus_macrophage_small.bioGPS.csv',
                            'default': True, 'id': 3, 'dir': b, 'name': 'GeneAtlas GNF1M, gcrma',
                            'delimiter': ',', 'color': 'GNF1M_plus_macrophage_small.bioGPS.coloring.csv',
                        ...
                        {
                            'ds': 'geneatlas_MOE430_20090327.raw.csv',
                            'default': True, 'id': 4, 'dir': b, 'name': 'GeneAtlas MOE430, gcrma',
                            'delimiter': ',', 'color': 'geneatlas_MOE430_20090327.coloring.csv',
...

                        {
                            ...
                            'delimiter': ',', 'color': 'GPL570_tissue_grouping_for_BioGPS.csv', 'owner': 'Michael Zilliox',
                        ...
                        {
                            'ds': 'tumors_U95_20070810.raw.csv',
                            'default': True, 'id': 7, 'dir': b, 'name': 'Primary Tumors (U95)',
                            'delimiter': ',', 'color': 'tumors_U95_20070810.coloring.csv',
                            'owner': '', 'summary': '',
                        ...
                        {
                            'ds': 'hypothalamus_msk.MOE430v2.csv',
                        ...
                        {
                            'ds': 'Pig_atlas_RMA_normalised_data_updated.csv',
                            'default': True, 'dir': pi, 'name': 'Pig Atlas',
                            'delimiter': ',', 'color': 'Pig_atlas_new_sample_annotation_updated.csv', 'owner': 'Tom Freeman',
                        ...
                        {
                            'ds': 'miRNA_fluid_profile3.csv',
                            'default': True, 'dir': mi, 'name': 'miRNA profiling in bodily fluids',
                            'delimiter': ',', 'color': 'miRNA_fluid_profile3.coloring.csv', 'owner': 'Andrew Su',
            ...
            for ds_cfg in datasets:
                upload_dataset(ds_cfg, stdout=self.stdout)


def upload_dataset(ds_cfg, stdout=None):
    ds = ds_cfg['ds']
    stdout = stdout or sys.stdout
    stdout.write('\n%s\n' % ds)
    #only comma or tab are the valid delimiter
    assert ds_cfg['delimiter'] in [',', '\t']
    ...
    db_platform = None
    platform = ds_cfg['geo_gpl_id']
    stdout.write('Platform: %s\n' % platform)
    plat_dict = {platform: []}
    ...
    try:
        ...
        stdout.write('DB Error - Dataset ID: %s, Platform: %s\n' % (dataset_id, platform))
    species = ds_cfg['species']
    stdout.write('Species: %s\n' % species)
    current_dataset = BiogpsDataset(id=dataset_id,
                                    name=dataset_name,
                                    ownerprofile=UserProfile.objects.get(user__username='asu'),
                                    platform=db_platform,
                                    summary=ds_cfg['summary'],
                                    species=species)

    with file(os.path.join(ds_cfg['dir'], ds), 'rb') as f:
        reader = csv.reader(f, delimiter=ds_cfg['delimiter'])
        line = 0
        for row in reader:
            ...
            if line == 0:
                ...
                color_file = ds_cfg['color']
                clr_file_vals = {}
                if color_file:
                    display_dict['color'] = ['color_idx']
                    display_dict['sort'] = ['order_idx']
                    with file(os.path.join(ds_cfg['dir'], color_file), 'rb') as c:
                ...
                metadata = {
                    "default": ds_cfg['default'],
                    "id": dataset_id,
                    "name": dataset_name,
                    "owner": ds_cfg['owner'],
                    "geo_gds_id": ds_cfg['geo_gds_id'],
                    "geo_gpl_id": platform,
                    "geo_gse_id": ds_cfg['geo_gse_id'],
                    "species": species,
                    "pubmed_id": ds_cfg['pubmed_id'],
                    "summary": ds_cfg['summary'],
                    "factors": factors_list,
                    "display_params": display_dict
                }
                # Metadata
                try:
                    _now = datetime.now()
                    ...
                    stdout.write('Metadata done.\n')
                except psycopg2.DataError:
                    stdout.write('DB Error - Dataset ID: %s, Metadata: %s\n' % (dataset_id, metadata))
                line += 1
                continue
            ...
            try:
                ...
                stdout.write('DB Error - Dataset ID: %s, Reporter: %s\n' % (dataset_id, reporter))
            line += 1

        stdout.write('Data done.\n')
    ...
    if db_reps:
        ...
        if db_count == ds_count:
            ...
            stdout.write('Number of reporters in dataset and DB don\'t match! DB: %s, DS: %s\n' % (db_count, ds_count))
    else:
        try:
            ...
            p.reporters = ds_reps
            p.save()
            stdout.write('Success saving new platform %s with %s reporters\n' % (platform, ds_count))
        except psycopg2.DataError:
            stdout.write('DB Error - Dataset ID: %s, Reporter: %s\n' % (dataset_id, platform))
